...
const ExpenseDashboard = () => {
    ...
    // Expenses data stores all the expenses stored in the database
    // The total count and today's count is calculated to display in Display Cards at the top
    ...
    const [totalExpensesForToday, setTotalExpensesForToday] = useState(0);
    ...
    useEffect(() => {
        ...
        const fetchExpenses = async () => {
            try {
                ...
                setTotalExpensesForToday(getTotalBasedOnTodaysExpenses(expenses));
                setTotalExpensesCount(getTotalExpensesBasedOnCurrentMonth(expenses));
    ...
    return (
        <div className="content">
            {userRole.includes('admin') || userRole.includes('user') ? (billsDataLoading && billsData && expensesDataLoading && expensesData ? (
                <p>Loading...</p>
            ) : (
                <>
                    <Row className='p-2 d-flex align-items-stretch'>
                        ...
                        <Col lg="4" md="6" sm="6" className="d-flex">
                            <DisplayCardsNoImage
                                cardTitle1={`Expenses - ${formattedDate}`}
                                cardMessage1={totalExpensesForToday}
                                footerMessage="Expenses Overview"
                                cardBackGroundColor="#ffffff"
                                cardTitle2={`Expenses in ${formattedMonth}`}
                                cardMessage2={totalExpensesCount}
                            />
                        </Col>
                    </Row>
                </>
                    ...
                    <Card>
                        <CardHeader>
                            <Row>
                                ...
                                <Col className="text-right">
                                    <Button color="primary" className='float-end' onClick={handleExpensesClick}>Add Expense</Button>
                                </Col>
                            </Row>
                        </CardHeader>
                        <CardBody>
