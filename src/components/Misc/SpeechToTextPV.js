...
const SpeechToText = () => {
    ...
    const parseTextToAPI = async () => {
        ...
        try {
            const prompt = `Parse the following medical text and extract information such as Name, Age, Gender, Complaints, Diagnosis, Examination, Instructions, and Medicines with details:\n\n${parsedText}`;
            const requestBody = {
                prompt: prompt,
                max_tokens: 1024, // Adjust this value based on how long you expect the summary to be
                temperature: 0.7, // Adjust for creativity, lower values make it more factual
            };
            const response = await axios.post(
